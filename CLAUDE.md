...
- `sim.py` — simulation classes only: `Model`, `Env`, `CEnv` (plus private `_PIDController`). Does `from .rbd import *` so inline references stay flat.
...
- Ray casts: `ray_intersects_{triangle,mesh,box,sphere,cylinder,capsule}` — used by `env.raycast` / `env.raymap`. Single-threaded; cost is O(pixels × raycast-shapes), brute-force per ray (no broadphase/BVH — meshes are O(triangles)/ray).
...
- `env.delete(name)` / `model.delete(name)` — splices the group's slots out of every parallel array, shifts trailing indices down (`parent`, `cbody`, `fbody`, `lock_idx`, `fixed`, `feeds[*]`, `fdict[*]`), pops fdict keys, rebuilds `X`/`I6`/`cpair` and recreates the C handle. `lam_prev` (LCP warm-start) reset to None — cpair size change invalidates it.
...
- **Backend scope**: tact-backend (`Env`) only. `CEnv` (mujoco/chrono/real wrappers) raises `NotImplementedError` from explicit `add`/`delete`/`groups` stubs — this also blocks the `__getattr__` forward-to-cdll footgun if a future backend happens to export a same-named C symbol. mujoco's mjSpec recompile path could be wired up later but isn't a priority.
