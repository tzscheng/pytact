...
./build.sh                   # builds bin/libtact.so from native/{rbd,shape,mpr,narrow,box_box,ray,lcp,tact,render}.c
                             ...
                             # also builds bin/mjenv.so from native/mjenv.cpp (auxiliary MuJoCo backend for
...
- `mjcf/*.xml` — shared MuJoCo **environment** xmls for `start -e <name> -m` (robot-independent; attached into the robot spec via mjSpec). `0.xml` = infinite checker ground plane, `hf1.xml` = hfield terrain (floor/terrain geoms group 1 = raycast convention)
...
./start -m cartpole          # MuJoCo backend (bin/mjenv.so): robot model = cartpole.xml next to the
...
./start -e env_yml cartpole           # add an environment scene on top of robot (tact: yml from cwd /
                                      # yml/ / envs/ / demos/; -m: xml from cwd then the shared tact/mjcf/)
...
All C/C++ sources + headers live under **`native/`** (headers beside sources, so `#include "tact.h"` needs no `-I`), built into `bin/libtact.so`. Linked deps: `-lm -lEGL -lGL -lGLEW -lGLU -lglfw -lturbojpeg -lzstd`.
...
Thin C++ shim around MuJoCo (`mjModel`/`mjData`, `mjv*` viz, GLFW window). Built into `bin/mjenv.so` and dlopened from `start -m`. Its only role is to cross-check `tact`'s dynamics against MuJoCo on the same YAML/XML pair (no per-robot code to host).
- `start -m` loads the robot model as `<module>.xml` next to the controller (`xml/<module>.xml` transitional fallback); `-e <name>` optionally names a **robot-independent environment xml** — searched in the project dir, then shared **`tact/mjcf/`** (`0.xml` = infinite checker plane, floor geom group 1 per the raycast/height_scan convention).
...
- `native/` — all C/C++ sources + headers (`rbd.c`, `shape.c`/`shape.h`, `mpr.c`, `narrow.c`, `ray.c`, `lcp.c`, `tact.c`/`tact.h`, `render.c`, `mjenv.cpp`). `build.sh` compiles these → `bin/`. Headers beside sources so `#include` needs no `-I`; nothing references them at runtime (the package loads `bin/libtact.so` only).
