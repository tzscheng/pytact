...
int egl_render(int n_obj, int* obj_type, float* shape, float* objcolor, float* objpose, float* campose, char* out_buf, int opt){
	...
	// Resolve target for MSAA pbuffer → single-sample glReadPixels.
	...
	glRenderbufferStorage(GL_RENDERBUFFER, GL_RGB8, egl_width, egl_height);
	...
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, egl_width, egl_height);
