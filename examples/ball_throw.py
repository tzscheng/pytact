...
wall every time you press SPACE.
    ...
    uv run python examples/ball_throw.py        # (run from the tact/ dir)
...
How it works: the ball is a free-flying sphere (examples/ball.yml) added to the
...
HERE = os.path.dirname(os.path.abspath(__file__))   # .../tact/examples
ROOT = os.path.dirname(HERE)                         # .../tact
sys.path.insert(0, os.path.dirname(ROOT))            # .../fg   (so `import tact` works)
os.chdir(ROOT)                                       # so 'examples/...' paths resolve
...
env = tact.Env('examples/box_wall', render=False)   # dt=0.001 from box_wall.yml (stable for this scene)
...
def throw():
    ...
    env.add('examples/ball', name=name)          # appended → its 6 DoF are q[-6:]
    balls.append(name)
