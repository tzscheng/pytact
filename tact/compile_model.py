...
def compile_model(src: str | os.PathLike[str], out: str | os.PathLike[str]) -> None:
    ...
    mesh_slots = {str(idx): path for path, idx in sorted(model.mesh_path_to_idx.items(), key=lambda kv: kv[1])}
    chunks.append(_chunk_text("mesh_json", json.dumps(mesh_slots, sort_keys=True, separators=(",", ":"))))
